...
- 진입점은 슬래시 커맨드다: `/design` → `/implement <NNN-slug>` → `/verify <NNN-slug>`.
  각 단계의 상세 절차와 금지 사항은 `.claude/commands/` 의 해당 파일에 있다.
- 한 세션에서는 한 단계의 역할만 수행한다. 각 단계의 산출물과 금지 사항:
  ...
  | 검증 | 설계 문서 §5·§7·§8, 코드 | `docs/plans/NNN-slug.verify.md` | 코드 수정, 실패 수정 |
